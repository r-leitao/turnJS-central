...
while($listeCat = $reqCat->fetch(PDO::FETCH_ASSOC))
{
	...
	while($listePDT = $reqPDT->fetch(PDO::FETCH_ASSOC))
	{
		if(!in_array($listePDT['referenceP'], $tabRefP))
		{
			...
			// echo '
			// mod::page::numP::nbPDT:::::::::'.$modele.'::'.$page.'::'.$numeroPageSommaire.'::'.$nbPDT.':::::'.$listePDT['designation'];
...
foreach ($sommaire as $cat => $stack) {
	...
	if($value[1]%2==1) $page=$value[1]+1;
		...
		// coupure si texte trop long dans sommaire
...
if(!empty($_SESSION[\'adherent\'][\'cle\'])){
	header(\'Location : http://www.canmk.fr/turnJS-central/index2.php\');
}
?>
';
?>
  ...
  <body style="background-color:#e7e7e7;">
        <nav id="navGlobal">
          ...
          <ul>
            ...
            <li><a href="http://www.canmk.fr/connexion.html">Connectez-vous</a></li>
...
<!--             <li><a href="#" ng-click="cacherTitre()">Cacher</a></li>
        ...
        <flipbook user-role="flipbook.html"></flipbook>

  </body>
</html>
<?php 
$contentHtml = ob_get_clean();
file_put_contents('index.php',$contentHtml);
